...
const DEFAULT_PRODUCTS_SINGLES_PATH = path.join(DATA_DIR, 'products_singles_6.json');

const DEFAULT_PRODUCTS_NONSINGLES_PATH = path.join(DATA_DIR, 'products_nonsingles_6.json');

const DEFAULT_PRICE_GUIDE_PATH = path.join(DATA_DIR, 'price_guide_6.json');
...
function findLocalCardmarketPricing(item, normalizedCard, productUrl) {
  ...
  const scored = candidates
    .map((c) => ({ product: c, score: scoreCandidate(c, item, referenceTrend, expectedExpansionIds) }))
    .sort((a, b) => b.score - a.score);

  let best = scored[0].product;

  // Tiebreaker: when multiple candidates score similarly, disambiguate
  // using expansion characteristics and variant ordering.
  if (scored.length > 1) {
    const topScore = scored[0].score;
    const tied = scored.filter((s) => topScore - s.score < 100);

    if (tied.length > 1) {
      const byExpansion = new Map();
      for (const entry of tied) {
        const expId = entry.product.idExpansion;
        if (!byExpansion.has(expId)) byExpansion.set(expId, []);
        byExpansion.get(expId).push(entry);
      }

      const setName = item.set_name || '';
      const isPromoLike = /\bpromo\b|\bjumbo\b|\boversized\b/i.test(setName);
      const isJumbo = /\bjumbo\b|\boversized\b/i.test(setName);

      let bestGroup = tied;
      if (byExpansion.size > 1) {
        const ranked = [...byExpansion.entries()]
          .map(([expId, entries]) => {
            const allProds = dataset.productsByExpansion.get(expId) || [];
            const nonSinglesCount = allProds.filter((p) => p.categoryName !== 'Pokémon Single').length;
            return { expId, entries, nonSinglesCount, totalSize: allProds.length };
          })
          .sort((a, b) => {
            // Promo/jumbo sets have zero or very few nonSingles
            if (isPromoLike) return a.nonSinglesCount - b.nonSinglesCount || b.totalSize - a.totalSize;
            // Regular cards: prefer larger expansions (main release sets)
            return b.totalSize - a.totalSize || b.entries.length - a.entries.length;
          });
        bestGroup = ranked[0].entries;
      }

      // Within the selected expansion group, sort by dateAdded (V-ordering)
      if (bestGroup.length > 1) {
        bestGroup.sort((a, b) => {
          const dateA = new Date(a.product.dateAdded || '9999').getTime();
          const dateB = new Date(b.product.dateAdded || '9999').getTime();
          return dateA - dateB;
        });

        if (productUrl) {
          // Use V-suffix from URL to pick the exact variant
          const vMatch = productUrl.match(/-V(\d+)(?:-|$)/i);
          const vIndex = vMatch ? parseInt(vMatch[1], 10) - 1 : 0;
          best = bestGroup[Math.min(vIndex, bestGroup.length - 1)].product;
        } else {
          // No URL: jumbo/oversized → later variant, otherwise first
          best = isJumbo ? bestGroup[bestGroup.length - 1].product : bestGroup[0].product;
        }
      } else {
        best = bestGroup[0].product;
      }
    }
  }
